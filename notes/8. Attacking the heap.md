...
In the following sections we break down how the glibc heap works by specifically looking at how to allocate data from a fresh heap, cache freed allocations, and how caches are used for re-allocating data from previously freed allocations.
...
> The glibc heap uses _inline metadata_ which is data included within a heap's allocation that provides information about the allocation or the cache to which it belongs. This data should never be touched by a user—it is there for reference by the glibc, and should only be used by glibc. That said, manipulating metadata using some of the vulnerabilities mentioned later in this section is a means of attacking the system.
...
Load [[tcache]] in GDB and break after freeing the allocations. Running `gef`'s `heap bins` command shows that the tcache bin index three is full.
...
0x555555559480:	0x0000555555559430	0x0000555555559010
...
int main() {
    ...
    // guard allocation to avoid consolidating big allocations together
    void* guard = malloc(0x20);
    void* big_allocation_2 = malloc(SIZE);
    // guard allocation to avoid consolidating into top chunk
...
```
./leak
Address of a: 0x55e245dda2a0
Address of b: 0x55e245dda2d0
Address of z: 0x55e245dda2d0
Data at z:    0x55e245dda2a0
```
...
What happens above if `d` is not allocated?
